...
const getters = {
  ...
  nextIncompleteQuestionId: (state, getters, rootState, rootGetters) => (quiz_id, question_index, loopCount = 0) => {
    if (loopCount > 1) return 'loop more than once' // Avoid infinite loop when no questions left incorrect.
...
const mutations = {
  ...
  setActiveQuizId: (state, id) => {
    ...
    if (!state.interactions[quiz_id]) state.interactions[quiz_id] = {}
  ...
  setActiveQuestionId: (state, id) => {
    ...
    if (!state.interactions[quiz_id][question_id]) state.interactions[quiz_id][question_id] = new Interaction()
  ...
  markCorrect: (state) => {
    ...
    state.interactions[state.active_quiz_id][state.active_question_id]['isCorrect'] = true
  ...
  incrementKeyPress: (state) => {
    ...
    state.interactions[state.active_quiz_id][state.active_question_id]['key_presses'] = newCount
    return state.interactions[state.active_quiz_id][state.active_question_id].key_presses
  },
  clearInteractions: (state) => {
    state['interactions'] = {}
    window.Vue.$gtag.event('gameplay_reset')
  }
}
